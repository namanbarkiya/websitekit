...
export const useBreadcrumbStore = create<BreadcrumbStore>()(
  persist(
    (set) => ({
      ...
      // Actions
      setBreadcrumbs: (items) => set({ items, isCustom: true }),
      addBreadcrumb: (item) =>
        set((state) => ({
      ...
      clearBreadcrumbs: () => set({ items: [], isCustom: false }),
      ...
      generateBreadcrumbs: (pathname: string) => {
        ...
        if (routeConfig[pathname]) {
          set({ items: routeConfig[pathname], isCustom: false });
          return;
        }
        ...
        for (const { pattern, generateItems } of dynamicRoutePatterns) {
          if (pattern.test(pathname)) {
            set({ items: generateItems(pathname), isCustom: false });
            return;
          }
        }
